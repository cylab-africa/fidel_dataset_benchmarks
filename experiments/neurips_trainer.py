...
for task in ["COMBINED", ]:
    ...
    tokenizer = CharacterTokenizer.from_pretrained(config["TOKENIZER"])
    ...
    train_data = MyCustomOcrDataloader(TRAIN_PATH, preprocessor=processor, tokenizer  = tokenizer, img_root=IMG_ROOT, transform=augmentor, max_seq_length= config["MAX_SEQ_LENGTH"])
    ...
    max_seq_length              = config["MAX_SEQ_LENGTH"] , # Max sequence length for transcripts. Check data verification.
    ).to(device)
    ...
    # checkpoint= torch.load("/home/admin/blessed/HandWritten_Amharic_English_OCR/dataset/checkpoints-basic-cnn-trocr/SYNTHETICcheckpoint-best-loss-model.pth")
    # model.load_state_dict(checkpoint['model_state_dict'])
    
    # checkpoint = torch.load("/home/admin/blessed/HandWritten_Amharic_English_OCR/dataset/checkpoints-basic-cnn-synth-cont-trocr/SYNTHETICcheckpoint-best-distance-model.pth")
    # checkpoint = torch.load("/home/admin/blessed/HandWritten_Amharic_English_OCR/dataset/checkpoints-basic-cnn-hand-warm-trocr/HANDWRITTENcheckpoint-best-distance-model.pth")
    # model.load_state_dict(checkpoint['model_state_dict'])
    WARM_START = config["WARM_START"]
    def warm_start_weights(model):
            for param in model.decoder.parameters():
                param.requires_grad = False

            # Unfreeze only the mha2 parameters (cross-attention in each decoder layer)
            for layer in model.decoder.dec_layers:
                for name, param in layer.mha2.named_parameters():
                    param.requires_grad = True
    def pre_train_weights(model):
    ...
    print("#"*10)
    print(f"Trainable Model Parameters:\n {para}")
    print("#"*10)
    ...
    # weights[tokenizer.eos_token_id] = 4.0  # e.g., reduce impact by 90%
    ...
    #     checkpoint_root = os.path.join(os.getcwd(), "checkpoints-basic-cnn-hand-trocr")

    os.makedirs(checkpoint_root, exist_ok=True)
    if USE_WANDB:
        wandb.watch(model, log="all")
    checkpoint_best_loss_model_filename     = task+f'_pretrain_{config["LANGUAGE_MODEL"]}' +'checkpoint-best-loss-model.pth' 
    checkpoint_best_distance_model_filename     = task +f'_pretrain_{config["LANGUAGE_MODEL"]}' +'checkpoint-best-distance-model.pth'
    ...
    for epoch in range(e, epochs):
        ...
        if epoch < 10:
            ...
            print("Warm Starting............")
            warm_start_weights(model)
            model.pre_train  = False
            checkpoint_best_loss_model_filename     = task+f'_pretrain_{config["LANGUAGE_MODEL"]}_warmstart_{True}' +'checkpoint-best-loss-model.pth' 
            checkpoint_best_distance_model_filename     = task +f'_pretrain_{config["LANGUAGE_MODEL"]}_warmstart_{True}' +'checkpoint-best-distance-model.pth'
            best_loss_model_path                    = os.path.join(checkpoint_root, checkpoint_best_loss_model_filename)
            best_distance_model_path                    = os.path.join(checkpoint_root, checkpoint_best_distance_model_filename)
        elif epoch == 15:
            print("Unfreezing all............")
            unfreeze_all(model)
            checkpoint_best_loss_model_filename     = task+f'_pretrain_{config["LANGUAGE_MODEL"]}_warmstart_{False}' +'checkpoint-best-loss-model.pth' 
            checkpoint_best_distance_model_filename     = task +f'_pretrain_{config["LANGUAGE_MODEL"]}_warmstart_{False}' +'checkpoint-best-distance-model.pth'
            best_loss_model_path                    = os.path.join(checkpoint_root, checkpoint_best_loss_model_filename)
            best_distance_model_path                    = os.path.join(checkpoint_root, checkpoint_best_distance_model_filename)
        ...
        if (epoch >1 )and (epoch % 4 == 0):    # validate every 2 epochs to speed up training
            levenshtein_distance, cers, wers, charf1, wordf1 = validate_fast(model, val_loader)
            if cers <= best_distance:
                best_distance = cers
    ...
    model.load_state_dict(checkpoint['model_state_dict'])
    result = {}
    for down_stream_task in [ "COMBINED","HDD","TYPED", "HANDWRITTEN", "SYNTHETIC", ][:]:
    # for down_stream_task in ["HANDWRITTEN",]:

        print(f'{task} is been evaluated on {down_stream_task}')
        VAL_PATH =config[down_stream_task+"_VAL_PATH"]
        IMG_ROOT = config[down_stream_task+"_IMG_ROOT"+"_TEST"]
        ...
        result[down_stream_task]  = {}
        for metric, score in zip (["lev", "cers", "wers", "charf1", "wordf1"], [levenshtein_distance, cers, wers, charf1, wordf1]):
            result[down_stream_task][metric] = score
        with open(f'vit_ocr_result{task}_.json', 'w') as file:
            json.dump(result, file, indent=4)
        print(f'results saved to-> vit_ocr_result{task}_.json')
