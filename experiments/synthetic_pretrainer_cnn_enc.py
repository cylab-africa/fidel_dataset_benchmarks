...
train_transforms = transforms.Compose([
    ...
    transforms.RandomAffine(degrees=0, translate=(0.02, 0.02), scale=(0.9, 1.1), shear=5),
...
train_data = MyCustomOcrDataloader(TRAIN_PATH, preprocessor=processor, tokenizer  = tokenizer, img_root=IMG_ROOT, transform=None)
...
def validate_fast(model, dataloader):
    ...
    for i, (inputs, targets_shifted, targets_golden, inputs_lengths, targets_lengths) in enumerate(dataloader):
        ...
        dist, cer, wer = calc_edit_distance(greedy_predictions, targets_golden, targets_lengths, tokenizer, print_example=True)
...
for epoch in range(e, epochs):
    ...
    if  (epoch % 5 == 0):    # validate every 2 epochs to speed up training
        levenshtein_distance, cer, wer = validate_fast(model, val_loader)
    ...
    if best_loss >= train_loss:
        ...
        # wandb.save(best_loss_model_path)
        print("Saved loss training model")
